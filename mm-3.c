 ...
 * mm.c
 *
 * explicit free list; boundary tag; best fit
 */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
...
// static long start_addr = 0;   /* heap start address */ 
#ifdef NEXT_FIT
...
static void *extend_heap(size_t words) 
{
    ...
    /* Check if bp is 1st free block in free list */
    char* next_free = GET_PTR(NEXT_FREE_BLK(heap_listp));
    if (next_free == bp) {
...
{ 
	...
	char* next_header = HDRP(NEXT_BLKP(bp));
        *(unsigned int*) next_header = 
            (*(unsigned int *) next_header) & 0xFFFFFFFD;
    }
}
/* $end mmplace */
      ...
      //  printf("PREV and NEXT IS FREE\n");
        if(PREV_BLKP(bp) == GET_PTR(NEXT_FREE_BLK(bp))){
         ...
         //   printf("neither block was first\n");
            ...
            //printf("the next free block of previous free block 
            //is the 3rd block\n");
             ...
             //   printf("the previous free block of next free block 
             //   is the 1st block\n");
...
static void printblock(void *bp) 
{
    ...
    if (hsize == 0) {
        printf("%p: Epilogue Block follows:\n", bp);
...
static void checkblock(void *bp) 
{
    ...
    if ((!halloc) && ((GET_SIZE(HDRP(bp)) != GET_SIZE(FTRP(bp))) 
...
//	bp = bp;
	...
	/* Check rover */
#ifdef NEXT_FIT
	if (!GET_ALLOC(HDRP(rover)) && rover != heap_listp) {
